...
%delay between keyboard input checks (in secs)
inputDelay = 0.1; % 0.2
...
while exitDemo == false
    ...
    if doOnce
        ...
        [~,keyCode] = KbWait( -3 ); %wait for key press
        keyPressTime = GetSecs;
    end
    ...
    confirmKeyText = fileread('instructions/confirmKey.txt');
    
    Screen('TextFont', var.w, 'Arial');
    Screen('TextSize', var.w, scaledSizeSmall);
    Screen('TextStyle', var.w, 1);
    DrawFormattedText(var.w, confirmKeyText, 'center', RectHeight(var.rect) - 3*forty);
    ...
    WaitSecs( 'UntilTime',keyPressTime+inputDelay );    
end
